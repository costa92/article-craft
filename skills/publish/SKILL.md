...
【合规 + 原创二选一】「创作来源-AI 生成」与「点亮原创」存在 gray
zone — 官方从未明确表态二者是否互斥(developers.weixin.qq.com 多个用
户提问帖无官方运营专员回复)。但用户社区共识 + 微信珊瑚安全公告口径暗
示: AI 完全生成 vs 原创声明实际操作中有张力。基于此走 A/B 二选一:

  路径 A — AI 辅助路径（推荐 article-craft 默认场景）
  ────────────────────────────────────────────────────
  适用: 主题/立场/数据/经历都是作者的, AI 起稿后人工审阅决策, 大量
       重写或补充. 不属于"完全自动化替代真人"(微信 2026-04 表态).

  [ ] 点亮「原创」声明 (≥ 300 字门槛, 已自动检查)
  [ ] 「创作来源」**不勾**「内容由 AI 生成」
       —— 文末 AIGC 显式标识 (Rule 18) 已涵盖 GB 45438-2025 硬合规
       —— 这是合规底线, 不需在「创作来源」重复声明
  [ ] 文末已包含 "AI 辅助起稿 + 人工核实改写" 等标识
       (Rule 18 自动检查)

  自检 3 问 (任一答 No 则不应走路径 A):
    1. 文中个人经历是真的吗? (我做 X 半年 / 我踩过 Y 坑)
    2. 文中判断/立场是作者本人的吗? (不是 LLM 默认观点)
    3. 发出去你愿意为里面所有判断负责吗?

  路径 B — AI 完全生成路径
  ────────────────────────────────────────────────────
  适用: LLM 一键生成 + 几乎不改 / 流水线批量产出 / 完全自动化.

  [ ] **不点**「原创」声明
       —— 严格合规, 避免被投诉"未授权再创作不能声明原创"
  [ ] 后台「创作来源 → 内容由 AI 生成」(4 选 1 单选)
       ⚠ 发布后不可修改、不可删除
       依据: GB 45438-2025 + 珊瑚安全 2025-08-31 公告
  [ ] 文末已包含 AIGC 显式声明 (Rule 18 自动检查)

  注意: 路径 B 也得过 4 项推荐池命中 (见下方), 但「原创」缺失会让
  推荐流量层级显著下降.

  路径 A vs B 的决策权: 作者本人. article-craft 不替作者做这个判断
  —— 工具只检测显式标识 (Rule 18) 是否存在, 不评估 AI 占比.
