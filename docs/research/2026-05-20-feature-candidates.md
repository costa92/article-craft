...
| **B3** | Real screenshot end-to-end snapshot tests | M–L | CI dep on Playwright (already required) | ✅ Phases 1+3 done v1.6.18/v1.6.19 (foundation + screenshot-e2e.yml workflow); Phase 2 in progress (7/15 platforms — github/hn/stackoverflow/x + reddit/zhihu/wechat via v1.6.21); Phase 4 queued |
...
| **B7** | Multi-provider image abstraction | M | Enables English-language output (D2) — ✅ Phases 1-3 done v1.6.17 / v1.6.20 / v1.6.22 (`ImageProvider` protocol + 4 built-in providers including self-hosted `sd-local` via a1111); Phase 4 queued |
| **B8** | Verify-claims expanded scope (flag validation) | M | Start with 5–10 high-frequency tools — ✅ **Phase 1 done v1.6.23** (7 tools: git/docker/kubectl/uv/npm/curl/python3 — long flags only, warnings non-blocking); Phases 2-3 queued |
