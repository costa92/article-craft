...
| 6. 时间参照 / 中文模糊量词 | `2026 年`、`6 月`/`6-8 月`（日历月 1-12，含区间）、"几起"、"数个" | "6 月 21 日更新" |

月份豁免带时长消歧（v1.9.12+）：前有时长动词（历时/耗时/花了/持续了/坚持了/长达 等）或后接"之久"的 `N 月` 是时长声明而非日历参照，照常 warning 并计入新颖桶——"历时 6 月才完成" 报，"持续到 6 月" 不报。
...
警告分两桶：**结构性计数**（单位为 个/篇/条/次/轮/组/文件/人/起——枚举文章自身范围内的事物，如"这 3 个技巧"）与**新颖声明**（测量值：%/倍/时间/容量/金钱/tokens/吞吐/行数）。两桶都逐条 warning，但只有新颖声明计入 `>5 高密度` 标记——防止结构性计数稀释真正需要核对的测量值信号。details 输出形如 `12 个未标注数字声明（新颖 7 / 结构计数 5）`。

例外（v1.9.12+）：`个` 量词后接时间词（月/小时/星期/周/天/分钟/秒）时是时长测量——"迁移花了 6 个月" 计新颖桶，不算结构计数。

已知取舍：人/起/次 等单位的大数值统计（"帮助了 3000 人"）也会落进结构桶、不触发高密度。田野数据里这些单位绝大多数是结构用法（"3 人团队"），按数值大小分流留待有误报实例后再做。
