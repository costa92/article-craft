...
2. **Python scripts under `scripts/`** — the deterministic helpers SKILL.md files shell out to. These handle things prompts can't reliably do: Playwright rendering, image-model API calls, image compression, CDN upload, cache files.
...
- `scripts/generate_and_upload_images.py` — batch processes `<!-- IMAGE: -->` / `<!-- PROMPT: -->` placeholders in an article.md, generates each image via `nanobanana.py` (walking `MODEL_FALLBACK_CHAIN`, Minimax-first with Gemini/OpenAI fallbacks), compresses with Pillow, uploads through PicGo or S3, and edits the article file in place. The `--process-file` flag is the standard invocation.
- `scripts/nanobanana.py` — single-image generation call walking the `MODEL_FALLBACK_CHAIN` from `config.py` (Minimax → Gemini → OpenAI).
...
- `scripts/config.py` — loads `~/.claude/env.json`, defines `MODEL_FALLBACK_CHAIN`, `cache_dir()`, tone tables. All configuration (image-model API keys, S3, timeouts) lives in `~/.claude/env.json` — see `ENV.md`.
