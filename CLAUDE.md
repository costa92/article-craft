...
- `scripts/bump_version.py` — updates `.claude-plugin/plugin.json` and all 11 `skills/*/SKILL.md` frontmatter (major/minor/patch). Does **not** currently touch `.claude-plugin/marketplace.json` — remember to bump that by hand or extend this script.
...
There is **no persistent cross-stage state file** as of v1.3.4. Stage status is tracked as a text block the orchestrator prints to chat, and `--upgrade` mode infers state heuristically from article.md content. See "Known design debt" below for the planned fix.
...
- **Version bumps**: `.claude-plugin/plugin.json` is the source of truth, but **13 files must move in lockstep** — `plugin.json`, `marketplace.json`, and all 11 `skills/*/SKILL.md`. Update `CHANGELOG.md` in the same commit. `.github/workflows/tag-release.yml` reads `plugin.json` on push to `main`; if the release for that version doesn't exist, it creates the tag + release (no auto-bump — that was a v1.3.2 bug fixed in v1.3.4). If the release already exists, the workflow is an idempotent no-op, so pushes without a version bump are safe.
