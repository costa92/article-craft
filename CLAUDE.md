...
- `scripts/bump_version.py` — updates `.claude-plugin/plugin.json` (major/minor/patch/explicit). Called manually or by the tag-release workflow.
...
There is **no persistent cross-stage state file** as of v1.3.0. Stage status is tracked as a text block the orchestrator prints to chat, and `--upgrade` mode infers state heuristically from article.md content. See "Known design debt" below for the planned fix.
...
- **SKILL.md frontmatter**: every skill must declare `name`, `version`, `description`, and `allowed-tools`. As of v1.3.0 all 11 skills comply — do not regress this. The orchestrator's `allowed-tools` list must stay a superset of the union of what downstream skills declare.
- **Skill versions**: plugin is at 1.3.0; most skills track it. `requirements` is intentionally ahead (1.3.1) because it evolves on its own cadence — leave it alone when bumping the others.
- **Version bumps**: `.claude-plugin/plugin.json` is the source of truth. `.github/workflows/tag-release.yml` reads that version on push to `main`, auto-creates a tag + GitHub Release, and auto-bumps the patch version if a release with the current version already exists. Update `CHANGELOG.md` in the same commit as the version bump.
...
These are architectural gaps identified in the v1.3.0 design review that were **intentionally deferred** because they require coordinated multi-file refactors. When you touch the adjacent code, consider taking one of these on:
...
- **Review Phase 2 auto-modify is underspecified**. "For dimensions <7/10, fix corresponding issues" is a creative-rewrite instruction, not a mechanical transform. The oscillation guard (break on non-improving score) added in v1.3.0 bounds the damage, but the real fix is to make Phase 2 **scoring-only** (diagnostic, not corrective), surface an actionable feedback list, and let the user decide whether to re-run write with targeted section hints. Removing the auto-modify loop would also eliminate the orphan-image risk noted in review's Rule 11 warning.
- **No persistent cross-stage state file**. `scripts/pipeline_state.py` used to exist as a `PipelineState` JSON helper but was never wired in — it was deleted in v1.3.0 cleanup. The gap remains: `--upgrade` mode uses text-heuristic detection against `article.md` (which lies on stale CDN URLs), and there is no crash-recovery path. Fix: design a `.article-craft-state.json` next to each article, have every skill read/write it as ground truth, and have `--upgrade` prefer it over text heuristics.
