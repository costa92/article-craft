...
There is **no persistent cross-stage state file** as of v1.3.2. Stage status is tracked as a text block the orchestrator prints to chat, and `--upgrade` mode infers state heuristically from article.md content. See "Known design debt" below for the planned fix.
...
- **SKILL.md frontmatter**: every skill must declare `name`, `version`, `description`, and `allowed-tools`. As of v1.3.2 all 11 skills comply — do not regress this. The orchestrator's `allowed-tools` list must stay a superset of the union of what downstream skills declare.
...
These are architectural gaps identified in the v1.3.2 design review that were **intentionally deferred** because they require coordinated multi-file refactors. When you touch the adjacent code, consider taking one of these on:
