...
对 v1.9.11（PR #26）的多 agent 对抗 review 复现了 4 个正确性缺陷，其中 3 个可修：
① 月份豁免只看 `full_match`，把 "历时 6 月" 这类紧凑时长声明**彻底静默**（改动前会
warning——时长恰是 Rule 24 要抓的可捏造测量值）；② `NUMERIC_CLAIM_PATTERN` 的单位
交替在 "6 个月" 里先捕获 `个`，时长被错分进结构桶，与文档"时间属新颖声明"的契约矛盾，
6 条捏造时长也不触发高密度；③ 月份区间 "6-8 月" 不在豁免正则覆盖内，正是本要消掉的
日历噪音却仍计新颖桶。第 4 个（人/起 大数值统计落结构桶）是有意取舍，记录在
self-check-rules.md，留待有误报实例再做数值分流。

### Changed

- 月份豁免从 `EXCLUDED_PATTERNS` 拆出为上下文感知的 `CALENDAR_MONTH_PATTERN`：
  覆盖区间形式（两端都须 1-12，"6-18 月" 仍报）；前有时长动词
  （`DURATION_CUE_BEFORE`：历时/耗时/花了/持续了/坚持了/长达 等）或后接"之久"
  （`DURATION_CUE_AFTER`）时不豁免——"历时 6 月" 报且计新颖桶，"持续到 6 月" 仍豁免。
- 分桶判定加 `GE_TIME_UNIT_AFTER`：`个` 量词后接时间词（月/小时/星期/周/天/分钟/秒）
  归新颖桶——"迁移花了 6 个月" 现在正确计入 >5 高密度阈值。
- 语料复测（39 篇窗口期文章）：警告 418 → 418（降噪不回退），新颖 272 → 274
  （两条 "N 个月" 时长正确改判），高密度文章 17 → 17。

### Added

- `tests/test_rule_24_noise_buckets.py` 新增 8 测试（月份区间 ×2、时长线索消歧 ×3、
  个+时间词分桶 ×3），全部先验证 RED 再实现。

### Docs

- `references/self-check-rules.md` Rule 24：豁免 6 补区间与时长消歧说明，分桶小节
  补 `个`+时间词例外及 人/起 取舍记录。
- `tests/test_rule_24_noise_buckets.py` docstring 田野数据统一为 39 篇 / 480 条
  （修正与 CHANGELOG/CLAUDE.md 不一致的早轮快照数字）。

## [1.9.11] - 2026-07-13 — Rule 24 降噪：月份豁免 + 高密度阈值分桶（关闭警告密度债务）

### Why

设计债务 "Rule 24 high warning-density tolerance" 的田野盘点（39 篇窗口期知识库文章，480 条警告）
发现债务描述已过时：版本号（`v1.7.4`）**早已**被 `NUMERIC_CLAIM_PATTERN` 的 `(?<![`\w\.])`
前置守卫挡住，真实噪音是 ① 日历月份（"6 月" 单模式 29 次，与已豁免的年份同类时间参照）和
② 结构性计数（"3 个"/"5 篇"，枚举文章自身范围内的事物，非可捏造测量值）。噪音稀释信号——
作者学会忽略高密度警告后规则失去价值。

### Changed

- `EXCLUDED_PATTERNS` 新增日历月份豁免（1-12 月；"18 月" 等非法月份仍报）。
- 高密度阈值（>5）只计**新颖声明**（%/倍/时间/容量/金钱/tokens/吞吐/行数）；
  `STRUCTURAL_COUNT_UNITS`（个/篇/条/次/轮/组/文件/人/起）仍逐条 warning 但不计入阈值。
  details 输出补充分桶计数：`N 个未标注数字声明（新颖 X / 结构计数 Y）`。
- 语料实测：警告 480 → 418（-13%），高密度文章 24 → 17（-29%），新颖声明信号完整保留。

### Added

- `tests/test_rule_24_noise_buckets.py`（10 测试：月份豁免 ×5、版本号钉子 ×2、分桶 ×3）。

### Docs

- `references/self-check-rules.md` Rule 24：豁免 6 扩为"时间参照"，新增分桶小节。
- `CLAUDE.md`：Rule 24 债务项移入 Closed（含田野数据）。
...
v1.9.6 文档声称"S2 英文短标签在所有模型（含 minimax）上都稳定渲染、无需 `--model` 覆盖"。
2026-06-04 端到端 dogfood（同一封面 prompt 跑 minimax vs gemini A/B）证明该说法**过度乐观**：
minimax 的文字保真度**随标签密度反相关**——6 标签密集封面主标签部分糊（`QDUVE`/`TEARNI`）、
箭头注文全 gibberish；而 gemini-2.5-flash-image 把同一密集封面的全部标签渲染清晰。4 标签稀疏
流程图 minimax 主标签能渲清。撤回错误结论，换成实证后的可操作建议。

### Fixed (docs)

- `image-guide.md` S2「📝 文字例外」：三条底线 → 四条，新增"**带标签的 S2 图用
  `--model gemini-2.5-flash-image`**" + 2026-06-04 A/B 实证段；删除"所有模型都稳/无需覆盖"的
  错误说法；明确"箭头只连线不写字"。
- `write/SKILL.md` S2 例外说明同步上述模型建议。
